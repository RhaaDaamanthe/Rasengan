...
<head>
    <meta charset="UTF-8">
    <title><?= isset($carte) ? 'Modifier' : 'Ajouter' ?> une carte anime</title>
    <link rel="stylesheet" href="/CSS/style.css">
</head>
<body>
    <h1><?= isset($carte) ? 'Modifier' : 'Ajouter' ?> une carte anime</h1>

    <form action="<?= isset($carte) ? '/admin/anime-cartes/modifier/' . $carte->getId() : '/admin/anime-cartes/ajouter' ?>" method="post" enctype="multipart/form-data">
        <label>Nom :</label>
        ...
        <label>Anime :</label>
        <input type="text" name="anime" required value="<?= $carte ? htmlspecialchars($carte->getAnime()) : '' ?>">
